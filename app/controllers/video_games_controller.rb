class VideoGamesController < ApplicationController
  ...
  def show
    @analyses = Analysis.all
    # Erreur : SQLite3::SQLException: no such column: analyses.video_game_id: SELECT "analyses".* FROM "analyses" WHERE "analyses"."video_game_id" = ?
    # id = params["id"]
    # @videogame = VideoGame.find(id)
    # @analysis = @videogame.analysis
  end
  ...
  private
    ...
    def video_game_params
      params.require(:video_game).permit(:id, :name, :editor, :img_url)
    end
end
